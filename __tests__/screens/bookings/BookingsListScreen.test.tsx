...
type MockBookingsListStore = jest.Mocked<
  Pick<
    BookingsStore,
    | 'setServiceCenterUuid'
    | 'fetchBookings'
    | 'clearError'
    | 'setSelectedDate'
    | 'goToNearestBookingDate'
    | 'getCardBackgroundColor'
  >
> & {
  selectedDate: Date;
  nearestBookingDate: Date | null;
...
describe('BookingsListScreen', () => {
  ...
  it('loads bookings on focus without token re-validation', async () => {
    ...
    expect(mockBookingsStore.setServiceCenterUuid).toHaveBeenCalledWith('sc-1');
    expect(mockBookingsStore.fetchBookings).toHaveBeenCalled();
  });
